...
* `<index_type>` - тип индекса, в настоящее время поддерживаются:
    ...
    * `json` — JSON-индекс для ускорения предикатов `JSON_EXISTS` и `JSON_VALUE` по колонкам типа `Json` и `JsonDocument`. Подробнее описан в [{#T}](../create_table/json_index.md).
