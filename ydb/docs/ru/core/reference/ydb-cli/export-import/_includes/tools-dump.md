...
- `--compression <STRING>`: Сжимает данные таблиц в том же формате, что и [`export s3`](../export-s3.md). Допустимые значения: `zstd` (уровень сжатия по умолчанию) и `zstd-N` (где `N` — уровень сжатия от 1 до 22). Сжатые файлы данных сохраняются как `data_XX.csv.zst`.
