...
Команда `tools restore` автоматически распознаёт сжатые файлы данных (`data_XX.csv.zst`), созданные [`export s3`](../export-s3.md) или [`tools dump`](../tools-dump.md) с опцией `--compression`.
