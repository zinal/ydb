...
* `--null-value STRING` — значение, которое будет импортировано как `NULL`. Опция доступна только для команд `import file csv` и `import file tsv`. Значение по умолчанию не задано: при отсутствии опции во всех колонках импортируемого CSV/TSV-файла ожидаются данные (пустые значения не интерпретируются как `NULL`).
