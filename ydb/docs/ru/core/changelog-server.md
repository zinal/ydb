...
* Улучшено удобство языка запросов: добавлена поддержка [`SHOW CREATE TABLE`](https://github.com/ydb-platform/ydb/issues/33) и [`SHOW CREATE VIEW`](https://github.com/ydb-platform/ydb/issues/16283), включая отображение настроек cache mode в генерируемом DDL.
* Улучшен оптимизатор запросов: реализованы [StreamLookupJoin без предварительного вычисления ключей левой части](https://github.com/ydb-platform/ydb/issues/27425) и [проталкивание фильтров в обе стороны `LEFT JOIN`](https://github.com/ydb-platform/ydb/issues/27558).
* Добавлен [режим кэширования в памяти (`CACHE_MODE`) для семейств столбцов таблиц](https://github.com/ydb-platform/ydb/issues/18691), включая поддержку в YQL и операционные ограничения.
* Реализованы [экспорт конфигурации топика в S3](https://github.com/ydb-platform/ydb/pull/18138) и [импорт конфигурации топика из S3](https://github.com/ydb-platform/ydb/pull/18214) ([#18195](https://github.com/ydb-platform/ydb/issues/18195)).
...
* Расширены возможности квотирования и наблюдаемости: в [RateLimiter добавлены помеченные (labeled) ресурсы](https://github.com/ydb-platform/ydb/issues/9730), что позволяет вести более детальный учет потребления по типам нагрузки.
* Возможности backup/restore в ветке 25.3 расширены: добавлены [локальные инкрементальные бэкапы v1.0](https://github.com/ydb-platform/ydb/issues/22043), [сохранение ACL](https://github.com/ydb-platform/ydb/issues/11452), [контроль целостности по checksums](https://github.com/ydb-platform/ydb/issues/11459) и поддержка [backup/restore для `VIEW`](https://github.com/ydb-platform/ydb/issues/12724).
...
* Переработана работа с метаданными системных представлений: [пути системных views теперь материализуются в SchemeShard](https://github.com/ydb-platform/ydb/issues/17039), что упрощает управление ACL и повышает согласованность API.
* Расширено управление секретами: в 25.3 включён [редизайн внутреннего secret manager](https://github.com/ydb-platform/ydb/issues/23194), включая schema secrets и более широкую поддержку `*_PATH`-интеграций для внешних источников данных.
* Для больших кластеров продолжено внедрение [Node Broker Delta Protocol](https://github.com/ydb-platform/ydb/issues/11064) в релизной линии 25.3.
* Расширены возможности управления кластером и хранилищем. Добавлены [настройки BS Controller в конфигурации кластера](https://github.com/ydb-platform/ydb/pull/17957), [базовая поддержка cluster bridging](https://github.com/ydb-platform/ydb/pull/18297) в рамках [Bridge mode (2dc)](https://github.com/ydb-platform/ydb/issues/27423), [реконфигурация Board через DistConf](https://github.com/ydb-platform/ydb/pull/18859) и [автоматическая реконфигурация StateStorage без простоя всей группы](https://github.com/ydb-platform/ydb/pull/17525).
* Для Distributed Storage добавлены новые параметры управления и размерности: [`GroupSizeInUnits` для storage groups и `SlotSizeInUnits` для PDisks](https://github.com/ydb-platform/ydb/pull/19337), [команда `ydb-dstool` для переноса PDisk между узлами без полной пересинхронизации данных](https://github.com/ydb-platform/ydb/pull/19684) и [поддержка параметра `--iam-token-file` в `ydb-dstool`](https://github.com/ydb-platform/ydb/pull/20303). Эти изменения также используются в рамках поддержки [малых дисков (8GB+)](https://github.com/ydb-platform/ydb/issues/27432).
* Улучшена надёжность BlobStorage в сценариях дефицита места: добавлено [безопасное троттлирование при Out of Space, переполнении лога и переполнении мелкими blob-ами](https://github.com/ydb-platform/ydb/issues/12510).
