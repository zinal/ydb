...
* `--null-value STRING`: The value to be imported as `NULL`. Available only for the `import file csv` and `import file tsv` commands. The option has no default value: if it is not set, data is expected in all columns of the imported CSV/TSV file (empty values are not interpreted as `NULL`).
