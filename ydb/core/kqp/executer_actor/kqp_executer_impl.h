    ...
    void HandleChannelData(NYql::NDq::TEvDqCompute::TEvChannelData::TPtr& ev) {
        ...
        if (StreamResult && txResult.IsStream && txResult.QueryResultIndex.Defined()) {
            ...
            if (!trailingResults) {
                ...
                protoBuilder.BuildYdbResultSet(*streamEv->Record.MutableResultSet(), std::move(batches), txResult.MkqlItemType, txResult.ColumnOrder);
                ...
                protoBuilder.BuildYdbResultSet(txResult.TrailingResult, std::move(batches), txResult.MkqlItemType, txResult.ColumnOrder);
